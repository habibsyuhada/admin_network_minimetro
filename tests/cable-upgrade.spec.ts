...
test("node inspector replaces a full-port cable and shows its peer icon and price difference", async ({
  ...
  await expect(page.getByRole("dialog")).toContainText("1/1 port terpakai");
  await card.locator("summary").click();
  await expect(card).toContainText("Bayar 100 gold");
  await page
    .getByRole("button", { name: "Ganti kabel 1 ke Fiber", exact: true })
    .click();
  await expect(page.getByTestId("gold")).toHaveText("1400 gold");
  await expect(card).toContainText("Fiber #1");
  await expect(card).toContainText("Kembali 100 gold");
  ...
  await page
    .getByRole("button", { name: "Ganti kabel 1 ke Ethernet", exact: true })
    .click();
